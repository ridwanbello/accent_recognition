...
def download_accentdb(accent_list: List) -> Tuple[List, List]:
    ...
    if(check_if_its_the_right_input_format(accent_list) == False):
        return f"Data not in the right format e.g {["american", "indian"]}"
    ...
    destination_dir = data_path / "audio_datas"
    os.makedirs(destination_dir, exist_ok=True)
    # Loop through each folder and copy it
    ...
    american_folders_to_delete = Path(destination_dir / "american")
    ...
    audio_files = list(parent_folder.glob("*/*.wav"))
